...
export function GradientButton({onClickInfo, handleClick, title}: Props) {
    return (
        <section
            onClick={handleClick}
            className="flex cursor-pointer relative items-center gap-[8px] px-[12px] py-[6px] md:px-[16px] md:py-[12px]"
        >
            <figure
            ...
            <a
                className="cursor-pointer text-text-sm text-tetriary transition-colors hover:text-secondary !text-secondary"
            >
                {title}
            </a>
            <button
            ...
            >
                <InfoIcon
                    className={classNames("h-3 w-3", styles.tabIcon, styles.tabIconActive)}
                />
            </button>
        </section>
    )
