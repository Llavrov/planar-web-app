...
export function Button({type = "primary", size = "m", disabled = false, icon, classNames = '', onClick, title = ''}: Props) {
    ...
    let hover = type === 'primary' ? 'hover:bg-primary-button-hover' :  type === "secondary" ? 'hover:bg-secondary-button-hover' : '';
    let height = size === "l" ? 'h-[62px]' : size === 'm' ? 'h-[49px]' : 'h-[49px] text-text-s';
